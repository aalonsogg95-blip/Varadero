...
if(!empty($nombre) && !empty($password)){
    ...
    $sql= mysqli_query($conn, "SELECT * FROM sesion WHERE ses_usuario ='{$nombre}' AND ses_contrasena = '{$password}' and ses_area='caja'");
    if(mysqli_num_rows($sql)>0){
         $row=mysqli_fetch_assoc($sql);
                ...
                $datos=[
                    "idusuario"=>"0",
                    "usuario"=>$row["ses_usuario"]
                ];
                 $_SESSION["datosUsuarioCaja"]=$datos;
                 echo "success";
