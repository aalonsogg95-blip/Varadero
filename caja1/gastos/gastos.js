...
continueBtnGastos.onclick=()=>{
    ...
    xhr.open("POST", "agregar_gastos.php", true);
    xhr.onload=()=>{
...
function buscarGastos(){
       ...
       $.ajax({
           ...
           data:{fieldMes,fieldAnual,fieldFecha,periodo},
    ...
    if(gastos.length>0){
       ...
       let table=document.createElement("table");
           ...
           for(g=0; g<gastos.length; g++){
               let {gas_id_gasto, gas_concepto, gas_proveedor, gas_costo, gas_fecha}=gastos[g];
...
/////////
//ELIMINAR
function eliminarGastos(gas_id_gasto){
    $.ajax({
        type:'POST',
...
function modalEditarGastos(stringGastos){
    let {gas_id_gasto, gas_concepto, gas_proveedor, gas_costo, gas_fecha}=stringGastos;
    $("#modalAgregarGastos").modal("show");
    ...
    form.querySelector('input[name=idgastos]').value=gas_id_gasto;
    form.querySelector('input[name=Gnombre]').value=gas_concepto;
    form.querySelector('input[name=Gdestino]').value=gas_proveedor;
    form.querySelector('input[name=Gtotal]').value=gas_costo;
    form.querySelector('input[name=Gfecha]').value=gas_fecha;
...
guardarBtnGastos.onclick=()=>{
    ...
    let xhr=new XMLHttpRequest();
    xhr.open("POST", "editar_gastos.php", true);
    xhr.onload=()=>{
        if(xhr.readyState===XMLHttpRequest.DONE){
            if(xhr.status===200){
                let data=xhr.response;
