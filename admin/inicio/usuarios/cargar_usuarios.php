...
$where = '';
if ($campo != null) {   
        $where = "WHERE ( nombre_usuarios LIKE '%" . $campo . "%') ";
}else{
...
$sql0=mysqli_query($conn, "SELECT COUNT(*) as conta FROM usuarios");
...
$where order by id_usuario desc $sLimit ");
...
if ($totalFiltro > 0) {
  $totalPaginas = ceil($totalFiltro / $limit);
    ...
    for ($i = $numeroInicio; $i <= $numeroFin; $i++) {
        ...
        $paginacion[] .= '<a class="page-lin" href="#" onclick="nextPageUsuarios(' . $i . ')">' . $i . '</a>';
        $paginacion[] .= '</li>';
    }
